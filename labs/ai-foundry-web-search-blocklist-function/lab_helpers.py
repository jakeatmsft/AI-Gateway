...
def build_function_package(lab_dir, destination):
    """Bundle Python 3.12 Linux wheels, even when the notebook runs on Windows."""
    uv = shutil.which("uv")
    if not uv:
        raise RuntimeError("Install uv and reopen the notebook before packaging the Function.")
    lab_dir, destination = Path(lab_dir), Path(destination)
    with tempfile.TemporaryDirectory(prefix="regex-function-") as directory:
        root = Path(directory)
        dependencies = root / ".python_packages" / "lib" / "site-packages"
        subprocess.run([
            uv, "pip", "install", "--target", str(dependencies),
            "--python-version", "3.12", "--python-platform", "x86_64-manylinux_2_17",
            "--only-binary", ":all:", "--prerelease", "disallow", "--link-mode", "copy",
            "--requirements", str(lab_dir / "src" / "requirements.txt"),
        ], check=True)
        with zipfile.ZipFile(destination, "w", zipfile.ZIP_DEFLATED) as package:
            for filename in ("function_app.py", "processing.py", "filtering.py", "metrics.py", "streaming.py", "relay.py", "host.json", "requirements.txt"):
                package.write(lab_dir / "src" / filename, filename)
...
def wait_for_function_indexing(resource_group, app_name, timeout=300):
    """Wait for an accepted package to start; never upload it again during warmup."""
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        try:
            functions = az("functionapp", "function", "list", "--resource-group", resource_group, "--name", app_name)
            routes = {item["name"].rsplit("/", 1)[-1] for item in functions or []}
            if {"redact", "responses"} <= routes:
                return functions
        except RuntimeError as exc:
            if not any(marker in str(exc) for marker in ("Bad Request", "GatewayTimeout", "ServiceUnavailable", "Read timed out")):
                raise
        time.sleep(10)
    raise RuntimeError("Function indexing is not ready. Inspect startup logs before publishing again.")
