...
from proxy import redaction as function_app
from proxy.test_filtering import document


class Request:
    def __init__(self, body, split=None):
        self.headers = {"x-proxy-key": "test-secret"}
        self.body = body
        self.split = split
...
def setup(monkeypatch):
    ...
    monkeypatch.setenv('PROXY_API_KEY', 'test-secret')
    return function_app.redact
...
async def test_regex_function_redacts_only_domains_without_a_model_call(setup, stream):
    ...
    assert result.headers['x-response-redaction'] == 'regex'
...
async def test_split_request_chunks_are_reassembled_before_any_sse_is_emitted(setup):
    ...
    class GatedRequest:
        headers = {"x-proxy-key": "test-secret"}
        async def stream(self):
...
async def test_json_and_streaming_terminal_responses_are_identical(setup):
    from proxy.search import stream_snapshot
    ...
    events = [json.loads(frame.split('data: ', 1)[1]) for frame in fragmented.split('\n\n') if frame.strip()]
    assert stream_snapshot(fragmented.encode()) == json.loads(json_result.body)
