#!/usr/bin/env uv run
####################################################################
# Sample TUI app with a push to talk interface to the VoiceLive API #
# If you have `uv` installed and an API key or Azure identity       #
# environment variable set, you can run this example with just     #
#                                                                  #
# `./voicelive-audio-test.py`                                      #
#                                                                  #
...
inference_api_path = os.getenv("INFERENCE_API_PATH") or "inference"
voice_live_endpoint = os.getenv("AZURE_VOICELIVE_ENDPOINT") or (
    f"{apim_resource_gateway_url.rstrip('/')}/{inference_api_path.lstrip('/')}"
    if apim_resource_gateway_url
    else None
)
voice_live_model = (
    os.getenv("AZURE_VOICELIVE_MODEL")
    or os.getenv("VOICE_LIVE_MODEL")
    or os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME")
    or "gpt-realtime"
)
voice_live_api_version = os.getenv("AZURE_VOICELIVE_API_VERSION") or os.getenv("INFERENCE_API_VERSION") or "2026-06-01-preview"
voice_live_voice = os.getenv("AZURE_VOICELIVE_VOICE") or os.getenv("VOICE_LIVE_VOICE") or "alloy"
api_key = os.getenv("AZURE_VOICELIVE_API_KEY") or os.getenv("API_KEY")
...
class RealtimeApp(App[None]):
    ...
    def __init__(self) -> None:
        ...
        if not voice_live_endpoint:
            raise ValueError(
                "Missing VoiceLive endpoint. Set AZURE_VOICELIVE_ENDPOINT or APIM_RESOURCE_GATEWAY_URL."
            )
        self.connection = None
        self.session = None
        self.credential = AzureKeyCredential(api_key) if api_key else DefaultAzureCredential()
        ...
        async with connect(
            ...
            async for event in conn:
                ...
                if event.type == ServerEventType.ERROR:
                    self._log(f"VoiceLive error: {event.error.message}")
                    continue
    ...
    async def _cancel_active_response(self, connection: Any) -> None:
        try:
            await connection.response.cancel()
        except Exception:
            # VoiceLive returns an error when there is nothing to cancel yet.
            pass
    ...
    async def send_mic_audio(self) -> None:
        ...
        try:
            while True:
                ...
                if not sent_audio:
                    asyncio.create_task(self._cancel_active_response(connection))
                    sent_audio = True
...
if __name__ == "__main__":
    ...
    try:
        ...
        if isinstance(app.credential, DefaultAzureCredential):
            asyncio.run(app.credential.close())
